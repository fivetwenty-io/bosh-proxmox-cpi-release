...
- SDN will be the default — it makes roaming from Chapter 6 safe
...
- And the routes will clean up after themselves: each advertised route will stamp a provenance tag on the VM, and delete_vm will remove the recorded subnets — refcounted, so paired routers sharing a route keep it until the last one goes; fail-open, so a cleanup error warns and never blocks the delete.
-->
