...
- Factory, not translator — manufactures primitives PVE doesn't offer
...
<!--
- Factory, not translator: PVE hands us a single-cluster hypervisor; we manufacture the cloud primitives it never offers — live node scoring, dual anti-affinity, network lifecycle, self-implemented idempotency. Surface coverage is real; the work is depth.
...
- Strict one-way dependency is the load-bearing claim: internal/pve never imports internal/agent, and agent never imports cpi. The PVE client knows nothing about agents; agents know nothing about dispatch.
- The one edge that looks like a cycle but isn't: config → hooks → cpi, while cpi does not import config. Hook config structs live in the hooks package deliberately to keep the graph acyclic — the build proves it.
...
- Rollback is LIFO and idempotent via sync.Once — a half-created VM unwinds VM teardown and LB deregistration together, surviving caller cancel through context-without-cancel.
-->
...
- The storage-lock timeout is PVE's wall, not our knob — a per-storage lockfile serializes every mutation; we absorb it with StorageLockBackoff (base 2s, cap 30s), but we cannot remove it.
-->
...
**The CPI is the small, stateless, honest factory that stands on the seam between a general orchestrator and a capable hypervisor, and manufactures on demand every cloud primitive neither side will provide.**
...
- What we build that PVE lacks: idempotency from scratch (no ClientToken or fingerprint CAS), capability-based sizing, and PVE-aware fault classification that reads Perl die() strings out of UPID task bodies — no reference CPI is this platform-aware because PVE leaks failure as strings, not HTTP codes.
- Coverage is complete: all 21 canonical CPI v2 methods with real logic plus the update_disk extension, and one of only two CPIs (with vSphere) to implement network lifecycle. Every remaining item is depth within a method, not a new stub.
- The honest close: what's open is deliberate and small — the DHCP and foreign-device half of IP-conflict detection (structurally blocked: PVE-API-only, no host shell), and multi-host API endpoint failover once a second node carries production load.
-->
