...
- Under `free` with the band moved away from where the parker lives (an unset band resolves to the built-in one under every strategy, so opting out alone strands nothing), both an attach and a delete must be refused: the CPI can no longer recognize the parker still holding the volume, so attaching would leave two VM configs referencing one volume and deleting would leave the parker's slot referencing storage that is gone. The moved band is derived at startup — the 1000-VMID window just above the lab's band, or just below it when the lab band ends too close to PVE's maximum VMID — and a band that fits neither window aborts the run with the fix (shrink or move `parked_disk_vmid_range_start`/`_end`) before anything touches the cluster.

- Under `free` with both band keys removed outright (the shape an operator lands in by reverting to `free` and deleting the properties), an attach must still drain the disk off the parker: the CPI resolves the built-in `90000`–`90999` band on its own. This step only proves something on a lab whose configured band is the built-in one; anywhere else the harness prints a `SKIP` naming the reason — the resolved band could not contain that lab's parker, which is exactly the moved-band shape the previous bullet already covers.

Along the way the pass confirms that the volume survives the park, that a second detach of an already-parked disk changes nothing, that `set_disk_metadata` skips the parker instead of writing to it, that `snapshot_disk` refuses a parked disk even with the band moved away from the parker (a PVE snapshot would take the whole parker and every other deployment's disk with it, and the holder is classified by its `bosh-parker` tag rather than by the band), that re-parking reuses the existing parker rather than creating a second one, and that `delete_vm` refuses the parker's own VMID.
