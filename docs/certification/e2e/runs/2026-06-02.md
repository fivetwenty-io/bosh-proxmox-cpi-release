# End-to-End Run Report: 2026-06-02

<!-- e2e-run-meta {"command": "all", "command_line": "BOSH_PVE_ENV=cpitest ./scripts/e2e all --clean --yes --with-artifacts --env cpitest", "date": "2026-06-02", "doc_suffix": "", "env": "cpitest", "exit_code": 0, "steps_failed": 0, "steps_passed": 41, "steps_skipped": 0, "verdict": "PASSED", "wall_hms": "24m50s"} -->

Authoritative clean (cold) end-to-end run of the Proxmox VE BOSH CPI on the
isolated `cpitest0` SDN network, driven by `./scripts/e2e all`. We wrote this
report by hand before the runner generated its own; later entries in this
directory follow the generated format, and lab addresses here are replaced
...
- **Symptom:** `create_vm` failed at lifecycle step 3 with `IP conflict detected — address <test-static-ip> is already statically assigned to VM 4255 ... on bridge/vnet "vmbr0"`. On the next attempt the same error named VM 4704.
...
- **Symptom:** run #4 failed at `bosh:create-env` with `Post "https://mbus:...@<director-ip>:6868/agent": dial tcp <director-ip>:6868: connect: operation timed out`.
...
- `cf ssh` port 2222 dropped by the Cloudflare-proxied wildcard; fixed with a dedicated DNS-only A record `ssh.<lab-domain> → <lab-ingress-ip>` — validated by `cf:ssh:e2e-smoke` returning command output (10.8s phase).
...
- **Lever — co-locate the artifacts store:** compiled-release fetches traverse `cpitest0` to `<artifacts-ip>:9000`; already on the isolated net, so low latency. No action unless the store moves off-net.
...
- Runs three scenarios sequentially (base, SDN network, snapshot-detach bypass). They share the single test IP (`<test-static-ip>`) and template, so parallelizing them would require distinct IPs and is not worth the complexity for an 11% slice.
...
## Measured results (post-optimization cold run #6, 2026-06-02)

Authoritative cold re-run with the optimizations committed (`acd57fb`): `BOSH_PVE_ENV=cpitest ./scripts/e2e all --clean --yes --with-artifacts --env cpitest` — started 08:23:45, finished 08:47:28. Artifacts: `.e2e-results/20260602-082345/`; console `.e2e-results/console/e2e-all-cold-20260602-082345.log`.

**41 pass / 0 fail, 23m42s (1422.4s)** — **-68s (-4.6%)** vs the 24m50s (1489.5s) baseline.

| Phase | Baseline | Run #6 | Δ | Note |
|---|---|---|---|---|
| teardown | 2m08s | 1m48s | -20s | variance (delete-env load), not an applied lever |
| lifecycle | 2m46s | 2m42s | -4s | ≈flat |
| artifacts:bootstrap | 26.9s | 10.5s, **off critical path** | -100% on critical path | overlap (#2) — ran concurrently under lifecycle, fully hidden |
| director bootstrap | 6m43s | 6m40s | -3s | create-env 299.0s (≈298.4s, clone-bound); emptyvm 93.9s vs 97.1s (-3.2s, watch-floor trim) |
| cf-deploy | 12m06s | **11m51s** | **-15s** | watch-time floor 5s→2s (#1) on the dominant phase |
| cf-push | 28.6s | 30.4s | +1.8s | variance |
| cf-ssh | 10.8s | 10.9s | ≈flat | |
| **Total** | **24m50s** | **23m42s** | **-68s** | all 41 steps PASS |

The applied levers behaved exactly as predicted: the artifacts overlap (#2) removed the full artifacts phase from the critical path, and the watch-time trims (#1, #4) shaved ~18s across cf-deploy + emptyvm. The remaining ~30s of the 68s delta is teardown variance. The conclusion holds: cf-deploy is clone/agent-bound, not knob-bound — further wall-clock reduction requires the trimmed-smoke trade-off (#3) or faster instance convergence, not more update-block tuning.
...
- This run was performed on the isolated `cpitest0` SDN vnet (zone `cpitest`, subnet `<lab-subnet>`), which eliminates the shared-LAN duplicate-IP/ARP-flap class that previously destabilized CF deploys.
