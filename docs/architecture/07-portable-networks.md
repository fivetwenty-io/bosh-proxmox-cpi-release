...
So managed SDN networking is a two-phase commit followed by a convergence gate. Every create or delete stages the change, applies it, and — on failure — rolls back exactly what *this* call staged and nothing more, on a context detached from cancellation so cleanup finishes even if the caller walks away. When the convergence poll is enabled, network creation waits until the vnet has actually appeared, and VM creation confirms each SDN-backed interface's bridge is present on the target node before attaching. A timeout there is reported as a retriable error, so the Director simply re-drives the call. The gate also distinguishes what it owns from what it must never wait on: a static bridge, which is pre-existing infrastructure, bypasses the poll entirely.
