...
The grant drops out only when both `vm_pool` and `stemcell_template_pool` are set to `""` — the preflight then makes zero PVE calls — and `cluster_lock_mode: pool`/`pool_reap_empty` are unused.
