...
The certification workflow can test a CPI release artifact before publication. Select the successful Candidate Release run, its artifact name, and the expected SHA256. The workflow downloads that artifact from this repository, checks the checksum, and reads the release name and version from `release.MF`.
...
The existing `skip_certify` and `skip_bats` inputs remain available for focused runs. A skipped suite supplies no evidence for that suite's release gate. All runs share the lab concurrency group described in [the scheduled certification workflow](scheduled.md).
