...
// scrubbed. The "@" must precede any "/", so a path-embedded "@" in a
// credential-free URL does not match.
var urlUserinfo = regexp.MustCompile(`([a-zA-Z][a-zA-Z0-9+.\-]*://)[^/@\s]+@`)

// sensitiveQueryParam masks the value of a URL query parameter whose name
// contains a sensitive fragment (e.g. ?access_token=, ?password=, ?sig= via
// "secret"/"token"). The fragment alternation is built once from
// sensitiveKeyFragments so the key-name and query-name secret vocabularies stay
// in sync. Case-insensitive; the captured prefix (delimiter + name + "=") is
// preserved and only the value is replaced.
...
func buildSensitiveQueryParamRegexp() *regexp.Regexp {
	...
	return regexp.MustCompile(`(?i)([?&][^=&#\s]*(?:` + alt + `)[^=&#\s]*=)[^&#\s]+`)
}
