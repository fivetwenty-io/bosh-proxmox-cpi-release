...
The CPI builds an ISO 9660 + Rock Ridge volume labeled `config-2`, writes the raw BOSH `settings.json` bytes to `/ec2/latest/user-data` (and `/ec2/latest/meta-data.json`), uploads it to the PVE storage pool, and attaches it to the VM as a CD-ROM on `scsi30` (see [SCSI slot usage](#scsi-slot-usage)).

This is the OpenStack ConfigDrive datasource layout. Stock OpenStack stemcells from bosh.io (for example `bosh-openstack-kvm-ubuntu-noble-go_agent`) recognise it on first boot without any `#cloud-config` parsing, `runcmd`, or `systemctl` step.

## Why no PVE-native sub-mode

An earlier `cloudinit_mode: native` setting attempted to deliver settings via PVE's `cicustom` + snippets mechanism. PVE's storage upload REST endpoint only accepts content types `iso`, `vztmpl`, and `import` — `snippets` requires SSH/filesystem placement to `/var/lib/vz/snippets/`. The CPI therefore drops the native sub-mode entirely. ConfigDrive-via-CD-ROM works on stock OpenStack stemcells without any cloud-init service running in the guest, which is strictly simpler and more reliable.
