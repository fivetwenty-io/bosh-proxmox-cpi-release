...
**CPI behavior.** Stemcell image and ConfigDrive ISO uploads resolve the target node's address from the `pve.node_endpoints` map first, then from `/cluster/status` discovery when `pve.node_endpoints_discovery` allows it (unset defaults to on exactly when `verify_ssl` is `false`; an explicit `true`/`false` overrides that default either way), and dial it directly for the upload, its task await, and the pre-retry sweeps. A direct dial that fails TLS certificate verification, a TLS handshake failure for any other reason, or the connection itself falls back to the configured endpoint with a warning and skips that node's direct route for the rest of the process. The uploads also carry their own retry budget (`pve.retry.storage_upload.max_attempts`, built-in 30 attempts: roughly six minutes on the transient curve, roughly twelve when storage-lock contention dominates) so a single create attempt outlasts a burst window the previous one-minute budget did not.

**Status.** Meets; map nodes explicitly in `pve.node_endpoints` on verifying deployments, and set `pve.node_endpoints_discovery: false` on clusters with a dedicated corosync ring rather than mapping every node just to suppress discovery.
...
**Best practice.** A detached persistent disk parked on a placeholder VM (the default `detached_disk_strategy: parked` model) must survive an accidental destroy of that placeholder just as reliably as an attached disk survives an accidental `delete_vm`.
...
**CPI behavior.** `pve.vm_pool` is validated at config load to differ from both `pve.stemcell_template_pool` and the reserved `bosh-lock-*` sentinel-pool namespace, so a manifest cannot configure a VM for two conflicting pool memberships. The same collision rules are enforced again on every name the resolution pipeline produces at `create_vm` time, so a `pve.vm_pool_template` render (or a `cloud_properties.pool` value) that happens to equal the stemcell template pool fails with a named error rather than landing a workload VM in the template pool. Both static properties default to a distinct, non-empty pool name (`bosh` for VMs, `bosh-templates` for stemcell templates) that the CPI creates on demand, so this separation holds even for a manifest that sets neither property explicitly.

**Status.** Meets.

**Resource pool layout: per-deployment defaults, create-if-missing, and default-on reaping.**
...
**CPI behavior.** `pve.vm_pool` (default `bosh`) and `pve.stemcell_template_pool` (default `bosh-templates`) are separate, create-if-missing pools: the CPI creates whichever one it needs the first time it is needed, tagging it with a `managed by bosh-proxmox-cpi` provenance comment, so no operator setup step is required before the first deploy. `pve.vm_pool_template` renders a per-deployment pool name from `{prefix}`, `{director}`, `{deployment}`, and `{instance_group}` when neither a call-level nor a `vm_type` `cloud_properties.pool` override is set, and its default `bosh-{director}-{deployment}` means an operator running several deployments against one PVE cluster gets a distinct pool per deployment out of the box, without setting `cloud_properties.pool` in any manifest. Recommended: keep the defaults unless the cluster's ACL design cannot grant pool creation (see [PVE API permissions — shared-cluster variant](pve-api-permissions.md#7-shared-cluster-variant-scoping-vm-mutation-to-a-resource-pool)), in which case set `pve.vm_pool_template: ""` to fall back to the single static `pve.vm_pool`.

**Status.** Meets, Configurable for the template layer and the reaper.

**Empty-pool reaping trade-off.**
...
**CPI behavior.** `pve.pool_reap_empty` (default `true`, matching the per-deployment pools the `pve.vm_pool_template` default creates) costs one extra `delete_vm`-time lookup: `delete_vm` confirms the pool carries the CPI's own provenance comment, refuses the static `pve.vm_pool` and `pve.stemcell_template_pool` by name regardless of comment, then attempts the delete and relies on PVE itself to refuse it if the pool is not actually empty; a not-empty or already-gone refusal is tolerated as an expected race rather than an error. The fast-path delete skips the reaper, so some empty pools persist until a later synchronous delete. Set it to `false` on a cluster where pools are also managed by other tooling and any automatic pool deletion is unwelcome.

**Status.** Configurable, on by default; per-deployment pools are removed with their deployments.

**Migration note.** `pve.vm_pool` and `pve.stemcell_template_pool` used to default to no pool assignment at all; they now default to `bosh` and `bosh-templates` respectively, created on demand. `pve.vm_pool_template` now defaults to `bosh-{director}-{deployment}` (per-deployment pools, previously disabled by default) and `pve.pool_reap_empty` now defaults to `true` (previously `false`), so those per-deployment pools are reaped with their deployments; `pool_reap_empty`'s comment check needs `Pool.Audit` alongside `Pool.Allocate`. A deployment upgrading without touching these properties starts assigning VMs into an auto-created pool per deployment and needs `Pool.Allocate` on the CPI's PVE token (see [PVE API permissions](pve-api-permissions.md)); `Permissions.Modify` is not required. Existing VMs sitting in the static `bosh` pool are adopted into their deployment's pool on the next `bosh deploy` (via `set_vm_metadata`); VMs in any other pool are left where they are. Set `pve.vm_pool_template: ""` to keep the single static pool (required on reduced-ACL clusters whose token cannot create pools), and `pve.vm_pool: ""` plus `pve.stemcell_template_pool: ""` to keep the original no-pool behavior. See [Configuration — Migration / Upgrade Notes](configuration.md#migration--upgrade-notes) for the full detail.
