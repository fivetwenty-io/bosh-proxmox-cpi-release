...
|---|---|---|
| `Sys.Audit` | `/` | All `info`, `has_vm`, `has_disk`, task polling, `GET /cluster/status` (also feeds VXLAN peer derivation for CPI-created zones), `/cluster/resources`, `/storage`, `/cluster/config/nodes`, `/nodes` (most handlers); HA-rule reads (`GET /cluster/ha/rules`, `/cluster/ha/resources`) when HA placement is enabled; `GET /cluster/firewall/options` — `create_vm.go`'s once-per-process datacenter firewall master-switch probe, triggered only when the VM being created requests `security_groups`, `pve.vm_firewall`, or `allowed_address_pairs` (see [Configuration — Firewall](configuration.md#firewall)); a probe failure (missing `Sys.Audit`, etc.) logs a warning and never fails `create_vm` |
...
| `Datastore.Allocate` | `/storage/<pool>` for each of `vm_storage`, `disk_storage`, `stemcell_storage`, `iso_storage`, and, unless `stemcell_replicate_storage_set` is false, every member of a bound storage set | `delete_disk.go`, `delete_stemcell.go` (`DELETE /nodes/{n}/storage/{s}/content/{volume}`) |
| `Datastore.AllocateSpace` | same storage paths | `create_disk.go`, `create_vm.go` (`import-from=<storage>:import/...` disk allocation) |
...
| `Datastore.Audit` | same storage paths | `create_stemcell.go`, `get_disks.go`, `has_disk.go` (list/check volume) |
...
| `Pool.Allocate` | `/pool` (parent, propagated, because per-deployment pool names are dynamic) | `pool.go`'s `EnsurePoolExists` (`POST /pools`), which is create-if-missing for every template-rendered per-deployment pool (`pve.vm_pool_template`, default `bosh-{director}-{deployment}`) and the static `pve.vm_pool` fallback (default `bosh`) at `create_vm` time, for `pve.stemcell_template_pool` (default `bosh-templates`) at `create_stemcell` time, for `pve.parker_pool` (default `{prefix}-parker`, which renders to `bosh-parker` on a stock deployment) on the best-effort sweep that follows every successful park, and again at `set_vm_metadata` time when pool reconciliation moves a VM (`PUT /pools/{poolid}` with `allow-move=1`); `cluster_lock.go` (`POST`/`DELETE /pools`, which create and delete the sentinel pools `bosh-lock-*` when `cluster_lock_mode: pool` is set, and on every park and unpark, where they serialize the parker's protection window under `bosh-lock-vm-<vmid>`; the lock is advisory there, so a denial degrades to an unserialized window with a warning rather than a failure); `delete_vm.go`'s empty-pool reaper (`DELETE /pools/{poolid}`, on by default via `pve.pool_reap_empty`). Required by default; set `vm_pool_template`, `vm_pool`, `stemcell_template_pool`, and `parker_pool` all to `""` and leave `cluster_lock_mode`/`pool_reap_empty` unused to avoid needing it. This privilege alone is sufficient, and no `Permissions.Modify` grant is needed; see the note below |
| `Pool.Audit` | `/pool` (parent, propagated) | `GetPoolComment` (`GET /pools`/`GET /pools/{poolid}`), which is probed at CPI startup by `preflightPoolAccess` (`cmd/cpi/main.go`) against the configured static pools, `pve.parker_pool` among them whenever the parked disk strategy is on; read by the post-park sweep, which asks the parker pool itself which parkers already belong to it; read by `cluster_lock.go` to check a lock pool's owner/expiry when stealing a lock, and again on every release to confirm the sentinel still belongs to the releasing caller (under `cluster_lock_mode: pool`, and on every park and unpark), and by `delete_vm.go`'s default-on empty-pool reaper (`pve.pool_reap_empty`) to check a pool's provenance comment before deleting it. Required by default, on the same condition as `Pool.Allocate` above. The startup preflight reads each static pool before the CPI serves its first request, and a denial (HTTP 401/403) on one of the two workload pools is fatal, while a denial on the parker pool logs a warning and lets the CPI boot |
...
**Storage-set replica note:** `create_stemcell` writes a cache template's root disk onto every member of the effective root storage set, so `Datastore.AllocateSpace`, `Datastore.Audit`, and `Datastore.Allocate` must cover each member's `/storage/<pool>` path as well as the four scalar pools. This applies by default to any deployment that binds a root or ephemeral set, so grant those paths whenever a set is bound rather than only when `pve.stemcell_replicate_storage_set` is spelled out. A member the token cannot reach is not an error. The replica build for that member fails, the fan-out logs one warning per member per upload, and every `create_vm` placed there clones in full instead of linked. Setting the property to `false` removes the requirement along with the replicas.
...
**`Pool.Allocate` note:** this privilege gates pool creation and deletion via `POST`/`DELETE /pools`. It is needed by default, because every pool the CPI uses is create-if-missing: the template-rendered per-deployment pools (`pve.vm_pool_template`, default `bosh-{director}-{deployment}`), the static `pve.vm_pool` fallback (default `bosh`), `pve.stemcell_template_pool` (default `bosh-templates`), and `pve.parker_pool` (default `bosh-parker`) are each created the first time they are needed, tagged with a `managed by bosh-proxmox-cpi` provenance comment. Because the per-deployment names are dynamic and PVE has no wildcard ACL path, grant `Pool.Allocate` at the `/pool` parent with propagation rather than on individual `/pool/<poolid>` paths. The obstacle is the names, not the pools' existence. PVE accepts an ACL on a `/pool/<name>` that does not exist yet (verified live, see §4), so a per-poolid grant works for pool names known ahead of time and only fails on the rendered ones. A deployment that sets `vm_pool_template: ""`, `vm_pool: ""`, and `stemcell_template_pool: ""`, and does not use `cluster_lock_mode: pool` or `pool_reap_empty`, should still grant `Pool.Allocate` when disks are parked, because parking and unparking take a `bosh-lock-vm-<vmid>` sentinel pool to serialize the parker's protection window, and because every successful park is followed by a sweep that puts the parker into `pve.parker_pool`. A denial on the lock pool is not fatal, because the window runs unserialized and logs a warning, but two concurrent windows on one parker can then leave a volume referenced by an `unusedN` key nothing else sees. A denial on the parker pool is not fatal either, and the parker simply stays out of the pool with a warning in the log. `PUT /pools/{poolid}` (`AddVM`, used by `create_stemcell.go` to assign the template VM, and `MoveVMToPool` with `allow-move=1`, used by `set_vm_metadata`'s pool reconciliation) additionally requires whatever permission-modification right governs the object being added to the pool; for VM members this is covered by `VM.Allocate` on `/vms`, already granted in §3 below, so no extra grant is needed beyond `Pool.Allocate` itself. **`Permissions.Modify` is not required** for any part of the CPI's pool lifecycle (create, assign, or delete). We verified that against a live PVE 9.2.4 cluster with a token restricted to exactly the privileges in this table (no `Permissions.Modify`), and pool creation, one-shot pool assignment on VM create, and the empty-pool reaper's delete all succeeded. PVE surfaces pool-create, pool-move, and non-empty pool-delete conflicts as HTTP 500 with descriptive text rather than the 409/404 a REST client might expect for those conditions, and the CPI recognizes them by matching known substrings in the error text (`already exists`, `belongs already to pool`, `is not empty`, `does not exist`) rather than by status code.

**`Pool.Audit` note:** required by default, on the same condition as `Pool.Allocate`. Before serving its first request the CPI runs `preflightPoolAccess` (`cmd/cpi/main.go`), which issues `GET /pools/{poolid}` (`GetPoolComment`) against each configured pool layer, `pve.vm_pool` (default `bosh`) and `pve.stemcell_template_pool` (default `bosh-templates`), to prove its identity can read a pool path before a real `create_vm`/`create_stemcell` hits an opaque denial. The preflight probes `pve.parker_pool` (default `bosh-parker`) as well whenever the parked disk strategy is on. A classified permission error (HTTP 401/403) on one of the two workload pools is fatal, and the CPI logs `pools preflight failed` and exits, so a deployment missing this grant never starts. The parker pool is the exception, because pool assignment there is best effort, and a denial on it logs a warning and lets the CPI boot. Other failures (network fault, PVE 5xx, timeout) are logged at Warn and treated as transient, and a pool that does not exist yet is not a failure. That is the normal zero-config first boot, and the pool is created lazily on first use.
...
The grant drops out only when `vm_pool_template`, `vm_pool`, `stemcell_template_pool`, and `parker_pool` are all set to `""`, `cluster_lock_mode: pool` and `pool_reap_empty` are unused, and nothing parks disks. The preflight then makes zero PVE calls.

Parkers have a pool of their own. `pve.parker_pool` names it as a template whose only token is `{prefix}`, and that prefix resolves from `pve.parker_prefix` when it is set, from `pve.vm_prefix` when it is not, and from the literal `bosh` when neither is set, so the default template `{prefix}-parker` renders to `bosh-parker` on a stock deployment. Every successful park is followed by a best-effort sweep, which starts once the guarded allocation window around that park has closed and talks to PVE through the CPI's own unguarded client. The sweep creates the pool when it is missing, giving it the same `managed by bosh-proxmox-cpi` provenance comment, lists the parkers on the node that share this deployment's resolved prefix, asks the parker pool itself which parkers already belong to it rather than trusting the cluster index, and assigns the ones that do not. The sweep therefore needs `Pool.Allocate` on the parker pool path to create the pool and to add members, and `Pool.Audit` to read it, both of which the propagated `/pool` grant in §3c already covers. Every failure in the sweep, a permission denial on the pool included, logs a warning and leaves the park itself successful, so an operator who grants per pool on `/pool/<name>` can either add the parker pool to that ACL or set `pve.parker_pool` to `""` and turn pool assignment off altogether. Renaming `pve.parker_pool` later does not move the parkers that already exist, because they stay in the pool they first joined, and only the parkers created after the rename land in the new pool.
...
3. Privileges: select `VM.Allocate`, `VM.Audit`, `VM.Clone`, `VM.Migrate`, `VM.Config.Disk`, `VM.Config.CDROM`, `VM.Config.Network`, `VM.Config.CPU`, `VM.Config.Memory`, `VM.Config.HWType`, `VM.Config.Options`, `VM.Config.Cloudinit`, `VM.PowerMgmt`, `VM.Snapshot`, `Datastore.Allocate`, `Datastore.AllocateSpace`, `Datastore.AllocateTemplate`, `Datastore.Audit`, `Pool.Allocate` and `Pool.Audit` (both required by default, because `vm_pool`, `stemcell_template_pool`, and `parker_pool` all default to a create-if-missing pool name, and the CPI's startup preflight reads each one under the default `parked` disk strategy; omit both only when all three pool names are set to `""`), `SDN.Allocate` (opt-in, needed only if `network_mode: sdn` is set, a network is marked `managed: true`, or `advertised_routes` is used), `Sys.Console` (if using HA placement features or DLB), and `Sys.Modify` (if using `placement.dlb.manage_cluster_crs`).
...
`Pool.Allocate` and `Pool.Audit` both belong in the default grant, because the per-deployment pools `pve.vm_pool_template` renders (default `bosh-{director}-{deployment}`), the static `pve.vm_pool` fallback (default `bosh`), `pve.stemcell_template_pool` (default `bosh-templates`), and `pve.parker_pool` (default `bosh-parker`) are all create-if-missing, and the CPI's startup preflight reads each static pool before serving a request, `pve.parker_pool` included under the default `parked` disk strategy. `SDN.Allocate`, `Sys.Console`, and `Sys.Modify` are opt-in only, since `network_mode: bridge` is the default and a default deployment makes zero SDN calls. A deployment that sets `network_mode: sdn` (or marks a network `managed: true`, or uses `advertised_routes`) needs `SDN.Allocate`; one that uses HA placement features or DLB needs `Sys.Console`; one that enables `placement.dlb.manage_cluster_crs` needs `Sys.Modify`. The two pool privileges drop out together, and only when `vm_pool_template`, `vm_pool`, `stemcell_template_pool`, and `parker_pool` are all set to `""` (with `cluster_lock_mode: pool` and `pool_reap_empty` unused). The minimum set for a bridge-mode deployment with no resource pools, no HA placement, and no DLB is `VM.*,Datastore.*` as listed in the privilege table above, plus `SDN.Use` on the bridge path via `BoshBridgeUse`, and that one applies to every deployment that attaches a NIC, bridge mode included. Note that the `VM.Config.*` list is wider than the CPI's own config writes suggest. PVE checks each config option's privilege class at submit time, so a VM create that sets `cores`, `memory`, and `scsihw` needs `VM.Config.CPU`, `VM.Config.Memory`, and `VM.Config.HWType` even though the CPI never updates those fields afterward (each was confirmed by a live `Permission check failed` denial when omitted). Note also that `delete_vm`'s `skiplock` flag is separate from this role. It is gated on the `root@pam` user, not on any privilege (see the `delete_vm` note above), so granting `BoshOperator` does not enable it.

### 3c. Grant ACLs

Five ACL grant categories apply by default: cluster-wide audit, VM operations, the bridge-attach grant (`SDN.Use` on the `localnetwork` zone, needed even in the default `network_mode: bridge`), one storage grant per configured storage pool, and the resource-pool grant at the `/pool` parent (needed by default, because per-deployment pools are created on demand with dynamic names, so a per-poolid grant cannot cover them). Two further categories are conditional: SDN (only when `network_mode: sdn` is set, a network is marked `managed: true`, or `advertised_routes` is used) and root-path (only when HA placement, DLB, or CPI-managed cluster CRS is used). The resource-pool grant drops out only when `vm_pool_template`, `vm_pool`, `stemcell_template_pool`, and `parker_pool` are all set to `""` and `cluster_lock_mode: pool`/`pool_reap_empty` are unused.
...
| `/pool` (parent, propagated) | `BoshOperator` | Required for `Pool.Allocate` and `Pool.Audit` by default. Every pool the CPI uses is create-if-missing: the per-deployment pools `pve.vm_pool_template` renders (default `bosh-{director}-{deployment}`), the static `vm_pool` fallback (`bosh`), `stemcell_template_pool` (`bosh-templates`), and `parker_pool` (`bosh-parker`). The CPI's startup preflight also reads the two static workload pools, and it reads `parker_pool` as well under the default `parked` disk strategy. The grant goes on the parent because the per-deployment names are dynamic and PVE has no wildcard ACL path; a `/pool/<poolid>` grant covers only the static names, so it boots clean and then fails at the first `create_vm`. Omit the whole grant only when `vm_pool_template`, `vm_pool`, `stemcell_template_pool`, and `parker_pool` are all set to `""`, and `cluster_lock_mode: pool` and `pool_reap_empty` are unused. For the single-pool reduced-ACL model, see §4 |
...
# vm_pool_template (bosh-{director}-{deployment}), vm_pool (bosh),
# stemcell_template_pool (bosh-templates), and parker_pool (bosh-parker)
# all create their pool on demand, and the CPI reads each static one at
# startup (a denial on vm_pool or stemcell_template_pool is fatal, so a
# role without Pool.Audit will not boot; a denial on parker_pool only
# warns). The grant goes on the /pool parent with propagation because the
# per-deployment names are dynamic; PVE has no wildcard ACL path. Omit the
# whole grant only if vm_pool_template: "", vm_pool: "",
# stemcell_template_pool: "", and parker_pool: "" are all set and
...
Every grant in §3 that lands on `/vms` is cluster-wide. On a cluster that also runs VMs the CPI does not manage, a `bosh@pve` token with those ACLs can mutate every guest, not only the ones it created. `pve.vm_pool` (see [Configuration](configuration.md)) closes that gap by assigning every workload VM the CPI creates to a dedicated resource pool at create/clone time, so the `/vms`-scoped privileges can be granted on `/pool/<bosh-pool>` instead.
...
| Path | Role | Notes |
|---|---|---|
| `/pool/<bosh-pool>` | `BoshOperator` | Replaces `/vms`. Covers `VM.Allocate` (create/clone into the pool), `Pool.Allocate` (create-if-missing for `<bosh-pool>` itself, since the CPI creates it on demand if it does not already exist; `Permissions.Modify` is not needed, see §2), and every other VM.\* privilege for VMs that are members of `<bosh-pool>`, which is every workload VM this CPI creates once `pve.vm_pool: <bosh-pool>` is set, with parkers going to `pve.parker_pool` instead |
...
| `/pool/<parker-pool>` | `BoshOperator` | This row only matters where a real `/vms` grant keeps parking available, because a pool-scoped token cannot park at all (see the caveats below). The sweep that follows a successful park creates `pve.parker_pool` on first use and assigns the node's parkers that belong to no pool yet, which needs `Pool.Allocate` and `Pool.Audit` on this path. The sweep is best effort, so a denial here logs a warning and the park still succeeds. Setting `pve.parker_pool: ""` turns the assignment off instead |

**Live-validated.** We exercised this exact table against a PVE 9.2.4 cluster with a `bosh-d3@pve` token (privsep=0) holding *only* the grants above plus §3c's audit, bridge-attach, storage, and `/pool` rows, with no `/vms` grant at all, and ran the full 16-step CPI lifecycle (`create_stemcell` through `delete_stemcell`, including disk create/attach/resize/snapshot/detach/delete and both reboot modes) with `detached_disk_strategy: free`. That run parked nothing, so it never exercised the parker-pool row. Every step passed: the create/clone pool-fallback works, and PVE's pool-permission propagation covers every subsequent config, status, disk, and snapshot operation on pool-member VMs, including template freeze and final cleanup. Re-run the §5 smoke test after any PVE upgrade all the same.
...
- **Set `detached_disk_strategy: free`.** The default `parked` strategy anchors every created and detached disk to parker VMs in a dedicated VMID band. Those parkers do join a resource pool of their own, the one `pve.parker_pool` names, but they join it through a sweep that runs after the park has already succeeded, so the membership arrives too late to help a pool-scoped token. The park creates the parker with its identity tags in the create call itself, and PVE checks those tags against `/vms/{vmid}` alone (see the create-time tag check note in §2), so `create_disk` still fails at the park step with a `/vms/<parker-vmid>` denial. We pass the tags at create on purpose, because tagging a parker only after it joined the pool would leave a window in which a VM that already holds a persistent disk carries no provenance tags at all. Under the reduced table, either set `pve.detached_disk_strategy: free` or keep a real `/vms` grant.
- **Cross-node disk migration is unavailable.** `disk_migrate.go` moves a disk by attaching it to a mover VM, which the CPI creates in the parker VMID band and deliberately leaves out of every resource pool, and then migrating that VM (`VM.Migrate` on `/vms/<mover-vmid>`). A pool-scoped token cannot drive it; deployments that rely on cross-node persistent-disk moves need the `/vms` grant.
...
## Multi-storage audit visibility

Extend storage grants to every configured set member, capacity-domain contributor, source, and ISO target. Keep audit access to historical stores until their allocations are reconciled. When ISO placement follows the selected root, every eligible root store that can receive an ISO needs the corresponding template-allocation privilege.

The CPI verifies effective audit visibility before certifying historical absence. It requires propagated `VM.Audit` at `/vms`, propagated `Datastore.Audit` at `/storage`, and `Sys.Audit` at `/access` for an unfiltered ACL inventory. It then checks explicit descendant restrictions and pool `NoAccess` rules. Image listings also require propagated `VM.Config.Disk` at `/vms` and that privilege on explicit VM paths, because PVE silently filters images that the caller cannot access. A propagated `Datastore.Allocate` grant covering every storage path satisfies that image-visibility check instead. The CPI observes these permissions; it does not change ACLs.

PVE filters its storage index according to per-store permissions. Its all-permissions response also omits paths with no permissions, so inspecting only that response cannot expose every denied path. The CPI queries relevant ACL paths individually. See the upstream [storage index implementation](https://github.com/proxmox/pve-storage/blob/master/src/PVE/API2/Storage/Config.pm), [effective-permission implementation](https://github.com/proxmox/pve-access-control/blob/master/src/PVE/RPCEnvironment.pm), and [ACL visibility check](https://github.com/proxmox/pve-access-control/blob/master/src/PVE/API2/ACL.pm).

Cluster continuity adds `GET /nodes/{node}/certificates/info`. The CPI reads the PVE root CA entry; it does not upload or replace certificates. The upstream [certificate information endpoint](https://github.com/proxmox/pve-manager/blob/master/PVE/API2/Certificates.pm) permits authenticated users to read that information.
