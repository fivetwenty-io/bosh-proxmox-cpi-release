...
Paths 2 and 3 also run unattended every Saturday against the `cpitest` lab, via `.github/workflows/certification.yml`. See [Scheduled Certification Workflow](scheduled.md) for its setup, its report-landing machinery, and its troubleshooting notes.
