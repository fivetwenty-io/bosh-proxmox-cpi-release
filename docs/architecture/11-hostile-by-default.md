...
So nothing reaches a log unscrubbed. A deep, structure-preserving scrub walks every argument and result before it is traced, masking sensitive-keyed values, URL userinfo, and signature-bearing query parameters. The *shape* stays intact, so the log stays diagnostic even with the secrets replaced by redaction markers. The redactor's coverage is not theoretical; it is the accumulated product of real leak post-mortems, each of which taught it one more place a secret hides.
...
Operators want side effects around CPI calls: audit logging, annotating a VM with its deployment identity, registering it in a load balancer, running a site-specific command. And they want them without forking the CPI or threading a conditional through every handler. These concerns are orthogonal to the per-method work, so the CPI factors them out as **hooks** — middleware wrapped around a stable handler interface. They run before and after each call without the handler knowing they are there. Configured hooks are resolved by name against a registry at startup, and an unknown name fails immediately rather than silently — a typo is caught at boot, not discovered in production.
...
Four hooks ship built in. An audit log records each call's duration and outcome but never its argument content, so it is redaction-safe by construction. A notes writer stamps the owning BOSH deployment into the VM's description, so the PVE UI shows who owns what. A load-balancer registration and a general host-command runner round out the set. They share one rule — side channels are best-effort. A load-balancer registration that fails is logged and moves on; it does not fail the VM lifecycle. An external dependency must never become a single point of failure for the core mutation. Only the core mutation itself is allowed to fail the call.

Two of those hooks dial directly into the hostile world, and each carries its own guard. The **host-command runner** treats its inputs as dangerous. There is no shell, so there is nothing for metacharacters to exploit. Executables come from an allowlist of absolute paths rather than a denylist, and an empty allowlist makes the hook entirely inert. Symlinks are resolved, so the allowlist cannot be tricked. The child's environment is scrubbed down to an explicit pass-list, so the Director's credentials never reach it. And a process-group kill on timeout means the child cannot orphan runaway grandchildren. Every dimension — path, environment, time, and process tree — is bounded, and the dangerous capability ships off.
