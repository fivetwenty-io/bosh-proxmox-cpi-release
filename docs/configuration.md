...
| `pve.stemcell_replicate_storage_set` | Boolean | `false` | When `true` and a root or ephemeral storage set is bound, `create_stemcell` builds one cache template per member of the effective root set (`root_storage_set`, else `ephemeral_storage_set`), each tagged `bosh-stemcell-storage-<storage-id>`, so a set-managed `create_vm` clones linked on whichever member placement chose. Replicas build after the primary and never fail the upload. A member that fails is logged and skipped, and `bosh upload-stemcell --fix` builds the missing ones. Replicas hold no director reference and are swept with the primary on its last reference. Requires `stemcell_strategy: template`, and the set's explicit member names must sanitize to distinct tags or the config is rejected. Overridable per cpi-config entry as `pve_stemcell_replicate_storage_set`. |
