...
The BOSH Proxmox CPI implements the full BOSH CPI v2 specification: 21 canonical methods plus `update_disk`, a PVE-specific extension inherited from the prior implementation.
...
The handler reads the VM's existing PVE tags, strips entries with the reserved prefixes `director--`, `deployment--`, `instance-group--`, `job--`, `index--`, and `vm-prefix--` (metadata key `instance_group` is emitted as `instance-group--<value>`, since PVE tags reject `_`), rebuilds those entries, and merges the result with any operator-supplied custom tags already on the VM. The first five come from the incoming metadata. The sixth is the identity tag, and it comes from the prefix `pve.parker_prefix` resolves to for this request, so a workload VM and the parkers holding its detached disks carry the same `vm-prefix--<value>` token. Custom tags from `create_vm` therefore survive Director re-syncs without manual reconciliation. The joined PVE tags string is capped at 350 bytes; entries that would overflow the cap are dropped at a whole-tag boundary rather than being cut mid-value. The CPI writes the identity tag last, so when a deployment and job name are long enough to overflow the cap, the identity tag is the first entry to go and every entry the Director supplied survives. Whenever the cap drops an entry, the CPI logs a warning that names the VM, the byte cap, and each entry it dropped, so the loss shows up in the logs instead of only in the PVE config. `set_disk_metadata` logs the same warning when a disk tag cannot fit beside the tags already on the hosting VM.

A workload VM picks the identity tag up from its first `set_vm_metadata` call, which the Director issues right after `create_vm`, rather than at create time. Stemcell templates never carry it.
...
- A parker VM inside the configured band: a stable-ID disk (a CID carrying the `bpd-` identity token) on the same node moves by reassignment — one `POST /nodes/{node}/qemu/{parker}/move_disk` that lands the volume directly on the target VM's slot, renamed for its new owner, with the drive serial and performance options riding along. A legacy disk, or a stable-ID disk still under its birth name on another node on shared storage, takes the config-edit path: the CPI detaches the disk from its parker so the normal attach path sees a free-floating volume. Unpark failure keeps the class the unpark chose and the disk remains safely parked. Most failures are retriable and the Director retries; a denied PVE grant and a reference the CPI could not sweep out of the parker's `unusedN` keys are permanent, and each names the command that repairs it. A stable-ID disk on a different node than the VM with no safe config-edit path (a parker-named volume, or any volume on node-local storage) is migrated to the VM's node by default (`pve.disk_migration: on_attach`): the disk rides a fresh single-purpose mover parker that is offline-migrated to the VM's node, attached from, and destroyed. With `pve.disk_migration: off` that case is refused, naming the knob and the operator escapes. See [Stable disk identity and ownership transfer](persistent-disk-strategy.md#stable-disk-identity-and-ownership-transfer).
...
**Stable-ID disks:** A disk whose CID carries the `bpd-` identity token detaches by ownership transfer instead: the slot is deleted (a raw config PUT, without the SDK's `unusedN` sweep — the sweep would deallocate a volume named for its holder), the resulting `unusedN` entry is reassigned to a parker via `POST /nodes/{node}/qemu/{vmid}/move_disk`, and the identity serial is re-applied on the landed slot. The parker's provenance record is written before the source slot is touched, so a crash anywhere in the window leaves a findable carrier and the Director's retry resumes the transfer. This transfer runs regardless of the configured strategy — a renamed volume cannot safely be left free-floating — so a stable-ID disk's detached state is always parked. See [Stable disk identity and ownership transfer](persistent-disk-strategy.md#stable-disk-identity-and-ownership-transfer).
...
If `metadata` contains a `tags` sub-object (map of `key: value`), the entries are extracted from the regular `bosh_disk_metadata` payload, written to the hosting VM's PVE tags field as sanitized `<key>--<value>` entries, and recorded separately in the description sentinel under `bosh_disk_tags[<disk_cid>]`. Existing tag entries whose key prefix collides with a new tag are replaced; other entries are preserved. An entry whose key is one the CPI reserves is the exception, and the CPI drops it with a warning rather than letting it replace the VM's own entry. The merged tag string is subject to the same 350-byte cap as `set_vm_metadata`, and when entries fall off the end to fit it the CPI logs a warning naming them. See [Custom Tags](configuration.md#custom-tags).
