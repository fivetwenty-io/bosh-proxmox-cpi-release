...
| `vm_pool_template` / `vm_pool` / `stemcell_template_pool` | `bosh-{director}-{deployment}` / `bosh` / `bosh-templates` | Every VM is assigned into its deployment's template-rendered pool (falling back to the static `bosh`), and every stemcell cache template into `bosh-templates`; the CPI creates each pool on first use (tagged `managed by bosh-pve-cpi`). A pool-creation or pool-assignment failure is fatal to `create_vm`/`create_stemcell`. | `Pool.Allocate` on `/pool` (parent, to cover dynamically named per-deployment pools) |
