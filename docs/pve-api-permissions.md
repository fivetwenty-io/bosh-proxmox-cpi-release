...
| `Datastore.Allocate` | `/storage/<pool>` for each of `vm_storage`, `disk_storage`, `stemcell_storage`, `iso_storage`, and, with `stemcell_replicate_storage_set` on, every member of a bound storage set | `delete_disk.go`, `delete_stemcell.go` (`DELETE /nodes/{n}/storage/{s}/content/{volume}`) |
...
**Storage-set replica note:** with `pve.stemcell_replicate_storage_set` on, `create_stemcell` writes a cache template's root disk onto every member of the effective root storage set, so `Datastore.AllocateSpace`, `Datastore.Audit`, and `Datastore.Allocate` must cover each member's `/storage/<pool>` path as well as the four scalar pools. A member the token cannot reach is not an error. The replica build for that member fails, the fan-out logs one warning per member per upload, and every `create_vm` placed there clones in full instead of linked.
