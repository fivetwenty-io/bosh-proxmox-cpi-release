...
When the `image_url` qualifies, we route it through PVE's own download-url API first, the same server-side transfer [Mode 3](#mode-3-server-side-download-heavy) uses: PVE streams the bytes directly into storage and the CPI never carries them, which also sidesteps upload proxying across cluster nodes. Four conditions must all hold: the URL is plain `https://`, `cloud_properties.sha256` is present (the server-side path derives filename, dedup, and CID identity from the digest), no credentials apply to the URL (neither `image_url_auth` nor a matching entry in the CPI config's centralized credentials, since PVE would not send them), and `cloud_properties.node` is unset (the server-side path resolves its own node, so a pinned node routes to the CPI-side fetch that honors it). Both paths derive the same digest-based filename and `:heavy:` CID, so if the server-side attempt fails before PVE accepts the download task we log a warning and fall back to the CPI-side fetch described here, with no change to the stemcell's identity. Once the task has started, there is no fallback: a CPI-side fetch would race a download that may still be writing the same file, so the error is returned instead — retriable when the task may still be running, and the Director's retried call picks the finished file up through dedup.
