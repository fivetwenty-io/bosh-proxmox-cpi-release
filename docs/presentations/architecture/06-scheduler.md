...
<!--
- PVE has no cross-node placement brain of its own — we will build one in the create_vm path, then hand the long-lived decisions back to PVE's HA rule store.
-->
...
- Asymmetric failure by design: only ListStatus (the node list) will be fatal; lose guest counts, per-node storage, or HA-maintenance state and we will degrade that axis and keep placing.
- Storage-headroom is just one scored axis — if a node's ListStorage call errors, we will drop the storage axis for that node rather than abort the whole placement.
- No standing scheduler to drift: we will gather facts fresh per create_vm. Set placement.enabled=false to fall back to static pve.node; an explicit target_node always overrides scoring.
-->
...
- Pin will write HA node-affinity rule — survives the process
- PVE's HA rule store will become our long-term memory
- PCI passthrough will force a strict pin
...
<!--
- The seam: our scorer will run once and the process will exit, but PVE HA can migrate a VM off our chosen node minutes later. The score alone has no memory.
- pin_az_via_ha_rules will write a per-VM bosh-na-{vmid} node-affinity rule pinning the VM to its AZ's node set — that survives our exit and keeps HA failover and DLB inside the AZ.
...
- PCI passthrough is incompatible with live migration, so any VM with pci_passthroughs will get a strict node-affinity pin automatically — non-negotiable, not operator-tunable.
- The per-VM pin needs no cluster lock (keyed on vmid, not a shared group); delete_vm will remove the rule, best-effort and idempotent. The pin is mutually exclusive with the DLB sentinel — a sentinel VM has no fixed AZ to pin to.
-->
...
<!--
- The honest admission: a one-shot scorer can't chase load that shifts after create. PVE 9.2's DLB is the continuous rebalancer we lack, so for HA guests we will register and step aside instead of pretending to balance.
- Two opt-in triggers: the master flag (every VM, still inside its AZ node set) or the sentinel AZ "dlb" — which will skip az_map and the scorer, land on any online node, register auto-rebalance=1, and deliberately skip the AZ pin.
