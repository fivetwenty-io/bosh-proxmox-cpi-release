...
| `/pool` (parent, propagated) | `BoshOperator` | Required for `Pool.Allocate` and `Pool.Audit` by default. Every pool the CPI uses is create-if-missing: the per-deployment pools `pve.vm_pool_template` renders (default `bosh-{director}-{deployment}`), the static `vm_pool` fallback (`bosh`), and `stemcell_template_pool` (`bosh-templates`), the last two of which the CPI's startup preflight also reads. The grant goes on the parent because the per-deployment names are dynamic and PVE has no wildcard ACL path; a `/pool/<poolid>` grant covers only the static names, so it boots clean and then fails at the first `create_vm`. Omit the whole grant only when `vm_pool_template`, `vm_pool`, and `stemcell_template_pool` are all set to `""`, and `cluster_lock_mode: pool` and `pool_reap_empty` are unused. For the single-pool reduced-ACL model, see §4 |
