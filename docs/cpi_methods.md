...
If `metadata` contains a `tags` sub-object (map of `key: value`), the entries are extracted from the regular `bosh_disk_metadata` payload, written to the hosting VM's PVE tags field as sanitized `<key>--<value>` entries, and recorded separately in the description sentinel under `bosh_disk_tags[<disk_cid>]`. Existing tag entries whose key prefix collides with a new tag are replaced; other entries are preserved. An entry whose key is one the CPI reserves is the exception, and the CPI drops it with a warning rather than letting it replace the VM's own entry. The merged tag string obeys the same 350-byte cap as `set_vm_metadata`, and when entries fall off the end to make it fit, the CPI logs a warning that names them. See [Custom Tags](configuration.md#custom-tags).
