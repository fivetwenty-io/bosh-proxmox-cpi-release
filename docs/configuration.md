...
| `pve.stemcell_storage` | Storage pool for stemcell qcow2 images. Must be a file-based PVE storage (`dir`, `nfs`, `cifs`, `glusterfs`, `cephfs`) — block-based storages (`lvm`, `lvmthin`, `zfspool`, `rbd`) cannot accept qcow2 uploads. On a multi-node cluster the pool must be shared, unless `vm_storage` is shared (for example Ceph RBD) and `stemcell_strategy` is `template`: then a node-local file pool is accepted for staging, and the single cache template serves every node via cross-node clone. Defaults to `vm_storage`; in that case `vm_storage` must satisfy the same constraints. | `""` (falls back to `vm_storage`) | no |
...
| `pve.vm_disk_format` | Disk image format (`qcow2`, `raw`, `vmdk`) for VM root disks, ephemeral disks, and persistent disks. Set `raw` when the target pool is block-native (`lvm`, `lvmthin`, `zfspool`, `rbd`), since those pools reject qcow2. A `vm_disk_format` or `disk_format` in a call's `cloud_properties`, or on a `vm_type` or `disk_type` profile, overrides this per disk. [Operations — Where the format comes from](operations.md) walks the full resolution order. | `qcow2` | no |
...
For multi-node clusters, `stemcell_storage` must also be shared, with one exception: when `vm_storage` is shared (for example Ceph RBD) and `stemcell_strategy` is `template`, a node-local file pool is accepted for staging. The qcow2 only needs to be reachable from the node that builds the cache template; the template's disk lands on `vm_storage` and clones to every node, so one template serves the whole cluster with no replication. In every other multi-node configuration, `create_stemcell` fails immediately with a descriptive error when the staging pool is local-only. Single-node clusters may use local file-based storage (e.g. the default `local` dir at `/var/lib/vz`); the shared check is skipped when the cluster reports exactly one node.
...
PVE resource pools group VMs (and, separately, stemcell templates) under a named identifier that can anchor ACL grants — see [PVE API permissions — shared-cluster variant](pve-api-permissions.md#7-shared-cluster-variant-scoping-vm-mutation-to-a-resource-pool). Out of the box, every deployment gets its own pool: the `pve.vm_pool_template` default renders `bosh-<director>-<deployment>` for each workload VM, `pve.vm_pool` (`bosh`) is the static fallback beneath it, and `pve.stemcell_template_pool` (`bosh-templates`) holds stemcell templates. The VM-side and template-side pools must always differ — pools are the CPI's ACL boundary, and sharing one pool would let a `create_vm`-scoped grant also touch stemcell templates — and this rule is enforced both at config load (`pve.vm_pool`) and against every name the resolution pipeline produces at `create_vm` time.
...
| 3 | `pve.vm_pool_template` (rendered) | Director-level template, default `bosh-{director}-{deployment}`; see below. |
| 4 | `pve.vm_pool` | Static global fallback, `bosh`. |
...
Any other `{...}` token in the template is rejected at config load. After substitution, repeated `-` separators collapse to one and leading/trailing `-` is trimmed; a render that collapses to `""` (every variable resolving empty, for example) falls through to the `pve.vm_pool` global default rather than producing an empty or malformed pool id. Under the default template, a `bosh create-env` run (no derivable director) renders `bosh-create-env`, shared by every create-env director on the cluster unless `pve.create_env_deployment` is set per bundle. Setting `pve.vm_pool_template: ""` disables the layer and keeps the single static `pve.vm_pool`.

### Membership provenance and reconciliation

Every VM that lands in a pool at `create_vm` time carries a `bosh_pool` record in its description's `<!--BOSH:{...}-->` sentinel block: the resolved pool name, which precedence layer produced it (`call`, `vm_type`, `template`, or `static`), and the template tokens (`director`, `deployment`, `instance_group`) derived at create time. On every `bosh deploy`, `set_vm_metadata` re-renders `pve.vm_pool_template` from those persisted tokens and, when the result differs from the VM's current pool, creates the target pool if needed and moves the VM into it. Only `template`-layer VMs are ever moved; a call-level, `vm_type`, or static pool choice is never overridden, and any failure in this reconciliation is logged as a warning without failing `set_vm_metadata`. A legacy VM without the record (created before this release) is adopted into the template flow only when it currently sits in the static `pve.vm_pool`; a VM in any other pool is left untouched.

### Create-if-missing and provenance

Both `pve.vm_pool` and `pve.stemcell_template_pool` are create-if-missing: the CPI creates the resolved pool the first time a VM or template needs it, tagging it with the comment `managed by bosh-proxmox-cpi` (plus ` (director <name>)` when the director name is derivable). Two CPI processes racing to create the same pool are both satisfied — PVE serializes pool creation, and the CPI treats a duplicate-create response as success.
...
`pve.parker_pool` names the PVE resource pool that holds the parker VMs of the parked disk strategy, and it behaves the way the other pools do. The CPI creates it the first time it needs one and writes the same `managed by bosh-proxmox-cpi` provenance comment into it. The resolved name has to be a flat poolid, and it has to stay clear of the reserved `bosh-lock-` cluster-lock namespace, exactly as a workload pool does. The property is a template whose only token is `{prefix}`, standing for the resolved parker prefix, and any other token is rejected at config load. A flat pool name given literally is accepted in place of the template, and setting `pve.parker_pool: ""` turns parker pool assignment off altogether.

The `{prefix}` of `pve.parker_pool` is not the `{prefix}` of `pve.vm_pool_template`. The [pool-name template](#pool-name-template) table above substitutes `pve.vm_prefix` for its `{prefix}`, while `pve.parker_pool` substitutes the parker prefix that `pve.parker_prefix` resolves to. One token spelling two different values is easy to trip over, so it is worth reading the table and this section together before we change either one. A deployment that sets `pve.vm_prefix` and nothing else gets that prefix on its new parkers as well, because the parker prefix falls through to `pve.vm_prefix` whenever `pve.parker_prefix` is unset, and a deployment that wants the two to differ sets `pve.parker_prefix` to something else.

Pool assignment runs after every successful park rather than only when the CPI creates a parker, and it is best-effort from end to end. The sweep looks at the parkers that belong to this director on the node it has just parked on, picks up the ones that belong to no pool, and moves those into the parker pool. A parker that already sits in some other pool is left where it is, a parker whose `director--` tag attributes it to another director is skipped, and so is a parker whose prefix is not the one this deployment resolved. A parker carrying no attribution tag at all is swept like any other, as long as its prefix matches. Membership is read from the parker pool's own member list rather than from the cluster index, which lags a pool change by minutes. Any failure in the sweep is logged as a warning and never fails the park, so a missing `Pool.Allocate` grant costs us the pool membership and nothing else. Because the sweep claims only parkers that belong to no pool, renaming `pve.parker_pool` leaves the existing parkers in the old pool until an operator moves them by hand.

Two smaller facts round the sweep out. The single-purpose mover VMs that `pve.disk_migration` builds never join the pool at all, since each one lives only for the length of one attach. At startup the CPI probes the parker pool alongside the other configured pools whenever the parked strategy is on, and a permission denial there logs a warning and lets the CPI boot, where a denial on one of the two workload pools is fatal. See [PVE API permissions](pve-api-permissions.md) for the grants involved.

A short row of guards keeps the parker pool to itself. The empty-pool reaper refuses it by name, the same way it refuses `pve.vm_pool` and `pve.stemcell_template_pool`, so a parker pool that sits empty between parks survives. A configuration whose parker pool renders onto `pve.vm_pool` or `pve.stemcell_template_pool` is refused at config load, and a workload pool that resolves onto the parker pool is refused at `create_vm` rather than quietly shared, because a workload VM in the parker pool would hold that pool open long after the deployment that created it is gone. The pool reconciler in `set_vm_metadata` refuses the same name as well, and there it warns and leaves the VM's pool alone instead of failing the call.

Parkers and workload VMs both carry a `vm-prefix--<value>` identity tag naming the prefix they resolved, so one tag filter in the PVE UI finds a deployment's VMs and the parkers holding its detached disks together. A workload VM picks the tag up from its first `set_vm_metadata` call, which the Director issues right after `create_vm`, rather than at create time. Stemcell templates never carry it.
...
`pve.pool_reap_empty` (default `true`) lets `delete_vm` clean up a pool once it becomes empty, so the per-deployment pools the template default creates disappear with their deployments. Before destroying the VM, the CPI records its pool membership; after the destroy completes, the CPI checks whether that pool carries the `managed by bosh-proxmox-cpi` provenance comment and, if so, attempts to delete it. A pool that is still non-empty, already gone, or missing the provenance comment is left alone, and the static `pve.vm_pool` and `pve.stemcell_template_pool` are refused by name regardless of comment — the reaper only ever removes per-deployment pools it created, and race conditions with another creator are tolerated silently. Only the primary `delete_vm` path reaps; the fast-path delete (`pve.fast_path_delete`) skips the reaper, so some empty pools persist until a later synchronous delete.
...
| Property | Type | Default | Description |
|---|---|---|---|
| `pve.stemcell_replicate_local` | Boolean | `false` | When `true`, `create_stemcell` uploads the qcow2 independently to every candidate cluster node's local storage and creates a per-node template VM tagged `bosh-stemcell-node-<node>`. Enables `create_vm` on local-storage nodes in multi-node clusters. `delete_stemcell` removes all replicas (best-effort; a single-node failure is logged and skipped). When `false` (default), local stemcell storage on a multi-node cluster is rejected at `create_stemcell` time, unless `vm_storage` is shared and `stemcell_strategy` is `template`: in that case a single cache template serves every node via cross-node clone and no replication is needed. |
| `pve.stemcell_replicate_storage_set` | Boolean | unset (→ on where it applies) | When a root or ephemeral storage set is bound, `create_stemcell` builds one cache template per member of the effective root set (`root_storage_set`, else `ephemeral_storage_set`), each tagged `bosh-stemcell-storage-<storage-id>`, so a set-managed `create_vm` clones linked on whichever member placement chose. Left unset this is on, because a root placed on any member other than `vm_storage` copies the whole stemcell across the network otherwise. It stays quiet where it cannot help. A deployment with no set bound, or one on `stemcell_strategy: import`, builds no replicas and reports no error, and a set whose only member is `vm_storage` builds nothing because the primary template already lives there. Set it to `false` to opt out and accept the full clones. Setting it to `true` also makes those preconditions mandatory, so a missing set or the `import` strategy becomes a config error, and explicit member names that sanitize to the same tag are rejected instead of skipped at build time. Replicas build after the primary and never fail the upload. A member that fails is logged and skipped, and `bosh upload-stemcell --fix` builds the missing ones. Replicas hold no director reference and are swept with the primary on its last reference. Overridable per cpi-config entry as `pve_stemcell_replicate_storage_set`, and an entry that names the key states an explicit choice for its cluster. |
...
| `pve.retry.disk_migrate.cap_ms` | Integer | `0` (→ 1800000 ms) | Wall-clock budget for awaiting the PVE migrate task the cross-node disk migration starts, sized by default for a node-local volume copy (30 minutes). When the budget runs out while the task still runs, `attach_disk` returns a retriable error saying the copy continues server-side; the Director's retried attach re-enters the migration and completes once it lands. |
...
| `pve.disk_migration` | String | `""` (→ `"on_attach"`) | Cross-node persistent-disk migration on attach. `""` or `"on_attach"`: when a stable-ID disk sits on one node and the target VM runs on another, `attach_disk` moves the disk instead of erroring. We isolate the disk onto a fresh single-purpose mover parker VM on its own node (a metadata-only reassignment, so sibling parked disks never travel), offline-migrate the never-started mover to the VM's node through the PVE migrate API (a metadata move on shared storage, a volume copy on node-local storage), attach from the mover, and destroy the now-empty mover through a guard that refuses to destroy a mover still referencing any volume. `"off"`: restore the hard cross-node errors; the refusal names this property. Legacy disks (created before stable disk identities) are never migrated under either setting, because the migration renames the volume and a legacy CID is the volume name; disks on an offline node cannot migrate either, since PVE needs the source node online. The migrate task's budget comes from `pve.retry.disk_migrate.*`. Overridable per cpi-config entry as `pve_disk_migration`. Valid values: `""`, `"on_attach"`, `"off"`. See [Persistent Disks](persistent-disks.md). |
| `pve.disk_cid_compression` | Boolean | `false` | Retained for compatibility; the behavior it opted into is now always on. When a disk CID's standard `pvd-` envelope would exceed 255 characters, the gzip-compressed `pvz-` envelope is emitted instead so the CID fits the `VARCHAR(255)` `disk_cid` column of MySQL-backed Directors (and the `dynamic_disks` table on every backend). CIDs that fit 255 characters stay `pvd-` and byte-identical, and the CPI decodes every previously emitted format unconditionally, so no disk migration is ever needed. See [Persistent Disks](persistent-disks.md). |
...
| `pve.parker_prefix` | String | `""` (→ inherited) | Prefix for parker VM names and, through the `{prefix}` token of `pve.parker_pool`, for the default parker pool name. A parker is named `<prefix>-parker-<vmid>`. The prefix resolves from this property when we set it, from `pve.vm_prefix` when we do not, and from the literal `bosh` when we set neither, so a deployment that leaves both alone keeps the `bosh-parker-<vmid>` names every earlier release produced. A deployment that already sets `pve.vm_prefix` gets that prefix on its new parkers unless it sets this property to something else, and the parkers it created before that change keep their old names and keep holding their disks. Those older parkers belong to the `bosh` prefix from now on, so the deployment fills fresh parkers of its own instead of them, and it sets this property to `bosh` when it wants the old ones to keep filling. A value set here has to match `^[A-Za-z0-9]([A-Za-z0-9-]*[A-Za-z0-9])?$`, stay within 46 bytes, and stay clear of the reserved `bosh-lock-` namespace. A prefix that arrives by inheritance from `pve.vm_prefix` is sanitized to the VM name character set and truncated to 46 bytes with a warning instead, since that property answers to rules of its own. Overridable per cpi-config entry as `pve_parker_prefix`. See [Parker VM properties](persistent-disk-strategy.md#parker-vm-properties). |
| `pve.parker_pool` | String | `{prefix}-parker` | PVE resource pool for parker VMs. The template accepts one token, `{prefix}`, which stands for the resolved parker prefix and not for the `{prefix}` of `pve.vm_pool_template`; any other flat pool name may be given literally instead. The default renders to `bosh-parker` on a stock deployment. The CPI creates the pool on first use with the same `managed by bosh-proxmox-cpi` provenance comment the other pools carry, and after every successful park it takes the node's parkers that share this deployment's resolved prefix and belong to no pool, and moves them into it, warning rather than failing when that move does not go through. Set `""` to turn parker pool assignment off. A rendered value equal to `pve.vm_pool` or `pve.stemcell_template_pool` is refused at config load, and a workload pool that resolves onto this one is refused at `create_vm`. Renaming this property leaves the existing parkers in the old pool until an operator moves them by hand. Overridable per cpi-config entry as `pve_parker_pool`. See [The parker pool](#the-parker-pool). |
...
| `pve.otel.service_name` | String | `"bosh-proxmox-cpi"` | Value of the `service.name` resource attribute attached to every emitted span, log record, and metric, identifying this CPI instance to the observability backend. |
...
**`pve.vm_pool_template` now defaults to `"bosh-{director}-{deployment}"`.** Previously empty (layer disabled). A deployment that upgrades and changes nothing now gets every new VM assigned into an auto-created pool per deployment (`bosh-<director>-<deployment>`; `bosh-create-env` on `bosh create-env` paths). Set `pve.vm_pool_template: ""` explicitly to keep the single static `pve.vm_pool` layout, which is required on reduced-ACL clusters whose CPI token cannot create pools (a single `/pool/<name>` grant with no wildcard ACLs); see [PVE API permissions](pve-api-permissions.md).

**`pve.pool_reap_empty` now defaults to `true`.** Previously `false`. Per-deployment pools are reaped once their last VM is destroyed on the synchronous `delete_vm` path; the static `pve.vm_pool` and `pve.stemcell_template_pool` are never reaped, and the fast-path delete skips the reaper. Set it to `false` explicitly to keep empty pools.

**Existing VMs in the static pool are adopted.** A VM created by an earlier release that currently sits in the static `pve.vm_pool` is moved into its deployment's template-rendered pool by `set_vm_metadata` on the next `bosh deploy`, and the derivation is persisted on the VM so later renders are stable. VMs in any other pool (a `cloud_properties.pool` override, an operator's own pool) are never moved.

**`pve.vm_pool` default changed from unassigned to `"bosh"`.** Before this release, an unset `pve.vm_pool` meant no pool assignment at all. It now defaults to `"bosh"`, and the CPI creates that pool automatically the first time it is needed; under the default template layer it is the fallback for VMs whose template render collapses to empty. To keep the old no-pool behavior, set both `pve.vm_pool: ""` and `pve.vm_pool_template: ""` explicitly.

**New ACL requirement for the default path.** Because pools are now created on demand — including one per deployment under the template default — the CPI's PVE token needs both `Pool.Allocate` and `Pool.Audit` in addition to the privileges documented in [PVE API permissions](pve-api-permissions.md), granted at the `/pool` parent so dynamically named per-deployment pools are covered, unless `pve.vm_pool_template`, `pve.vm_pool`, and `pve.stemcell_template_pool` are all explicitly set to `""`. `Pool.Audit` is required because the CPI reads each configured static pool at startup to fail fast on a missing grant; a denial there is fatal, so a token without it will not boot. Template-rendered pool names cannot be probed at startup, so a missing `/pool` grant there surfaces as a named, non-retriable `create_vm` error instead. `Permissions.Modify` is not required — verified against a live PVE 9.2.4 cluster.
...
**Templates keep their membership.** Stemcell templates created before this upgrade keep whatever pool membership (or lack of it) they already had; only workload VMs in the static `pve.vm_pool` are subject to the adoption above.
...
- The CPI reserves six tag-key prefixes: `director--`, `deployment--`, `instance-group--`, `job--`, `index--`, and `vm-prefix--`. It rebuilds all six on every `set_vm_metadata` call, the first five from the metadata the Director supplies and the sixth from the prefix `pve.parker_prefix` resolves to. Custom tags under any other key survive those re-syncs untouched.

- A reserved key on a `disk_type` is dropped rather than applied, and the CPI logs a warning naming the key and the disk. A reserved key on a `vm_type` is not dropped at `create_vm` time, and the next `set_vm_metadata` call rebuilds the entry from the CPI's own inputs and discards the operator's value. Either way the operator's value does not survive, so a custom tag of our own needs a key the CPI does not reserve.
...
| `vm_pool_template` / `vm_pool` / `stemcell_template_pool` | `bosh-{director}-{deployment}` / `bosh` / `bosh-templates` | Every VM is assigned into its deployment's template-rendered pool (falling back to the static `bosh`), and every stemcell cache template into `bosh-templates`; the CPI creates each pool on first use (tagged `managed by bosh-proxmox-cpi`). A pool-creation or pool-assignment failure is fatal to `create_vm`/`create_stemcell`. | `Pool.Allocate` on `/pool` (parent, to cover dynamically named per-deployment pools) |
