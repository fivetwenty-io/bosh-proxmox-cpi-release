...
| `pve.cpu_type` | Emulated CPU type/model written to every new VM's `cpu` config key (e.g. `x86-64-v2-AES`, `host`). Empty (default) resolves to `x86-64-v2-AES` — PVE's own creation-wizard default since 8.0: keeps AES-NI (the API-level `kvm64` fallback strips it, a fleet-wide performance tax for TLS-heavy BOSH workloads) and live-migrates across CPU generations from roughly 2010 onward. The sentinel `pve-default` writes no `cpu` key at all, restoring the `kvm64` fallback. `cloud_properties.cpu_type` (per-instance-group, resolved through the same layered `vm_type`/`disk_type` resolver as other create_vm knobs; the sentinel works there too) overrides this global value; `cloud_properties.pve_config.cpu` is a separate raw escape hatch applied after VM creation and always wins as the final write. For hardware older than the `x86-64-v2-AES` baseline, set the cluster's lowest-common-denominator named model. Avoid `host` wherever live migration is possible — it exposes the exact physical CPU feature set and crashes on migration to a differently-generationed node. PVE validates the model name itself; the CPI passes the value through verbatim. | `x86-64-v2-AES` | no |
