...
func resolveDiskPerfOptions(r *layeredResolver, cfg *config.CPIConfig) (map[string]string, error) {
	...
	if resolveDiskPerfBool(r, "iothread", diskPerfCfgBool(dp, func(d *config.DiskPerformanceDefaults) *bool { return d.Iothread })) {
		opts["iothread"] = "1"
	}
	if resolveDiskPerfBool(r, "ssd", diskPerfCfgBool(dp, func(d *config.DiskPerformanceDefaults) *bool { return d.SSD })) {
		opts["ssd"] = "1"
	}
