...
Parkers carry a `vm-prefix--<value>` tag too, naming the prefix `pve.parker_prefix` resolved to, and the parker prefix and the parker pool are both per-entry overridable, as `pve_parker_prefix` and `pve_parker_pool`. Giving an entry its own parker prefix is what makes its parkers tellable apart by name and by tag from a sibling entry's, which matters most when two entries point at one PVE cluster. Two entries pointing at separate clusters may carry the same parker pool name without interfering, because a resource pool lives inside a single cluster, while two entries against one cluster share a pool unless we name one of them differently. None of this separates parkers the way the VMID bands do. A park still chooses its parker by the `bosh-parker` tag, the configured band, and the director attribution, so a prefix and a pool are labels rather than boundaries.
