...
| `disk_format`        | String  | PVE disk format: `qcow2` (file storages) or `raw` (required for `lvm`/`lvmthin`/`zfspool`). This key outranks the `pve.vm_disk_format` job property, so a pool that holds raw volumes you did not ask for is almost always carrying a `disk_format: raw` here. [Operations — Where the format comes from](operations.md) walks the full resolution order. |
