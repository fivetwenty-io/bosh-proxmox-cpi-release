...
| Path | Role | Notes |
|---|---|---|
| `/pool/<bosh-pool>` | `BoshOperator` | Replaces `/vms`. Covers `VM.Allocate` (create/clone into the pool), `Pool.Allocate` (create-if-missing for `<bosh-pool>` itself — the CPI creates it on demand if it does not already exist; `Permissions.Modify` is not needed, see §2), and every other VM.\* privilege for VMs that are members of `<bosh-pool>` — i.e. every workload VM this CPI creates once `pve.vm_pool: <bosh-pool>` is set, with parkers going to `pve.parker_pool` instead |
