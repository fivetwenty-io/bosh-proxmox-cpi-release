...
def compiled_releases_ops(
    releases: list[CompiledRelease],
    stemcell: "str | None" = None,
    generated_by: str = "scripts/bosh compile-releases",
    consumer: str = "create-env",
) -> str:
    """Generate the create-env ops YAML pinning each compiled release.

    ``generated_by``/``consumer`` only shape the header comments so the file
    reads correctly for its caller (scripts/bosh create-env vs scripts/cf
    deploy); they do not affect the ops themselves.
    ...
    lines = [
        ...
        f"# Pins compiled release tarballs so {consumer} skips package compilation.",
