...
- Storage uploads no longer traverse the cross-node pveproxy hop, and they carry their own retry budget. An upload targeting storage on a node other than the configured endpoint was proxied by the endpoint node's pveproxy to the target node's pveproxy, and under a many-VM deploy burst that hop shed connections mid-request (`EOF`) while uploads to the endpoint node itself kept working; the roughly one-minute upload retry window then ran out before the burst did. Stemcell image and per-VM ConfigDrive ISO uploads now dial the target node's own pveproxy directly, resolving its address from the new `pve.node_endpoints` map (node name to `host` or `host:port`; explicit entries always win) and falling back to `/cluster/status` discovery when `verify_ssl` is `false`, since the discovered corosync address is usually absent from stock node certificate SANs. A direct dial that fails TLS certificate verification logs a warning and falls back to the configured endpoint, so a wrong entry degrades to the proxied path instead of failing the deploy. The uploads also moved to a dedicated retry class, `pve.retry.storage_upload.max_attempts`, whose built-in budget of 30 attempts (roughly six minutes on the transient curve) outlasts a burst window the previous 10-attempt default did not.
